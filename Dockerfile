...
RUN  python3 -m venv /opt/venv && . /opt/venv/bin/activate && pip3 install -r requirements.txt && pip3 install scalyr-agent-2 && ls /tmp/ && ls /opt/venv/lib/python$(python3 --version 2>&1 | sed -re 's/^Python ([0-9]+\.[0-9]+)\.[0-9]+$/\1/')/site-packages/scalyr_agent/builtin_monitors/ && mv /tmp/${testFile}.py /opt/venv/lib/python$(python3 --version 2>&1 | sed -re 's/^Python ([0-9]+\.[0-9]+)\.[0-9]+$/\1/')/site-packages/scalyr_agent/builtin_monitors/${testFile}.py && cd /opt/venv/lib/python$(python3 --version 2>&1 | sed -re 's/^Python ([0-9]+\.[0-9]+)\.[0-9]+$/\1/')/site-packages/ && if [ -z "$CONFIG_VALUES" ] ; then python3 -m scalyr_agent.run_monitor scalyr_agent.builtin_monitors.${testFile}  ; else python3 -m scalyr_agent.run_monitor -c "$(echo $CONFIG_VALUES)" scalyr_agent.builtin_monitors.${testFile} ; fi
